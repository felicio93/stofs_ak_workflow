...
def gen_model_configure_month(cfg: dict, ym: str):
    ...
    content = re.sub(r"^(start_year\s*:\s*).+$", r"\\1{year}", content, flags=re.MULTILINE)
    content = re.sub(r"^(start_month\s*:\s*).+$", r"\\1{month}", content, flags=re.MULTILINE)
    ...
    content = re.sub(r"^(nhours_fcst\s*:\s*).+$", r"\\1{nhours}", content, flags=re.MULTILINE)
    content = re.sub(r"^(dt_atmos\s*:\s*).+$", r"\\1{dt_atmos}", content, flags=re.MULTILINE)
