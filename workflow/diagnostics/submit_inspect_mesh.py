...
This step is grouping-agnostic — it runs once regardless of the grouping
mode and writes output to D{ID}/D{ID}_fix/. No changes to the core logic
are needed; this file is updated only to use model_dir from the updated
config module.
"""
...
def submit_inspect_mesh(cfg: dict, config_dir: Path) -> str:
    ...
    if sentinel.exists():
        print("  inspect_mesh already complete (sentinel found). "
              "Skipping.")
        return ""
    ...
    out_str = submitter.render_and_submit(
        "inspect_mesh.sbatch", subs,
