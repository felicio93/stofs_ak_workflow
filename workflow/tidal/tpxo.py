"""
tpxo.py — TPXO9 tidal database reader and interpolator.
Copied from pyschism (MIT licence) and adapted to remove the pyschism
package dependency. Only requires netCDF4, numpy, scipy.
"""
import logging
import os
import pathlib
...
# Minimal base class (replaces pyschism.forcing.bctides.base.TidalDataProvider)
class TidalDataProvider:
    pass
...
# Default location matching pyschism convention: ~/.local/share/tpxo/
_DEFAULT_TPXO_DIR = pathlib.Path.home() / ".local" / "share" / "tpxo"
    ...
    def h(self):
        if not hasattr(self, '_h'):
            if self._h_file is None:
                ...
                if self._h_file is None:
                    self._h_file = _DEFAULT_TPXO_DIR / TPXO_ELEVATION
    ...
    def uv(self):
        if not hasattr(self, '_uv'):
            if self._u_file is None:
                ...
                if self._u_file is None:
                    self._u_file = _DEFAULT_TPXO_DIR / TPXO_VELOCITY
