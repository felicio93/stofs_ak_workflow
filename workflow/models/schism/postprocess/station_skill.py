...
observations for every valid station in fix/station.in.
"""
...
def _load_model_staout(cfg, staout_num: int, stations: list,
    ...
    for ym in months:
        ...
        # Read the actual run start from param.nml.  For all months except the
        # first, gen_param enforces start_day=1 and start_hour=0.  The first
        # month preserves the template values (which may have start_day=7,
        # start_hour=12 for a mid-month cold start, etc.).
        year  = int(ym[:4])
        month = int(ym[4:])
        start_day  = 1
        start_hour = 0
...
def _obs_for_variable(source, tok, cfg, sid, start_str, end_str):
    """Return observed series (or dict for wind) for a station/variable."""
    import pandas as pd
...
def run_station_skill(cfg: dict, config_dir=None):
    ...
    print(f"  {len(coops)} CO-OPS + {len(ndbc)} NDBC station(s)   "
          f"Output: {out_dir}")
    print(f"{'='*60}\n")
