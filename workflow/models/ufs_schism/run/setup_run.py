...
FIX_LINKS = [
    ...
    "rough.gr3", "estuary.gr3", "TEM_nudge.gr3",
...
def _render_diag_sbatch(cfg: dict, mdir: Path, rdir: Path,
    ...
    subs = {
        ...
        "PARTITION":          slurm.get("partition",
                                        "hercules-2"),
        "NDIAG_VARS":         str(nvar),
        "MEM":                slurm.get("diag_run_mem",
                                        "16G"),
        "WALLTIME":           slurm.get("diag_run_walltime",
                                        "00:10:00"),
        "LOGDIR":             str(mdir / "logs"),
        "MAILUSER":           slurm.get(
            "mail_user", "felicio.cassalho@noaa.gov"),
        "PY":                 env_python(
            cfg, "diag_run_plots", default="swf_plot"),
        "SCRIPT":             (
...
def _render_combine_output_sbatch(cfg: dict, mdir: Path,
                                   rdir: Path,
                                   group_index: int) -> Path:
    slurm          = cfg.get("slurm", {})
    pid            = cfg["project_id"]
    combine_nranks = int(slurm.get(
        "combine_output_nranks", 80))
    combine_nodes  = max(1, (combine_nranks + 79) // 80)
    jobname        = f"CO{pid}_{group_index + 1:02d}"
    ...
    subs = {
        ...
        "COMBINE_WALLTIME": slurm.get(
            "combine_output_walltime", "04:00:00"),
        "LOGDIR":           str(mdir / "logs"),
        "RUNDIR":           str(rdir),
        "MAILUSER":         slurm.get(
            "mail_user", "felicio.cassalho@noaa.gov"),
...
def _setup_group(cfg: dict, mdir: Path, group_id: str,
    ...
    if not idir.is_dir():
        print(f"  ERROR {group_id}: input dir not found: "
              f"{idir}")
        return False
    ...
    def _check_sentinel(sentinel_path: Path,
        ...
        if not sentinel_path.exists():
            ...
            print(f"    Re-run:  stofs-ak --run --only "
                  f"{step} --config <cfg>")
            return False
        return True
    ...
    if not _link(idir / datm_subdir, rdir / "forcing"):
        print(f"  ERROR {group_id}: forcing dir missing: "
              f"{idir / datm_subdir}")
        return False
    ...
    if modulefiles_src.is_dir():
        if not _link(modulefiles_src,
                     rdir / "modulefiles"):
            print(f"  WARNING {group_id}: could not symlink "
                  f"modulefiles/.")
    else:
    ...
    if group_index == 0:
        if not _link(idir / "hotstart.nc",
                     rdir / "hotstart.nc"):
            print(f"  ERROR {group_id}: first-group hotstart "
                  f"missing: {idir / 'hotstart.nc'}")
    ...
    for name in OUTPUT_PLACEHOLDERS:
        ...
        if not f.exists():
            f.touch()
    shutil.copy2(bind / combine_exe,
                 outdir / combine_exe)
    shutil.copy2(bind / combine_output_exe,
                 outdir / combine_output_exe)

    # --- nhot_write from param.nml ---
    nhot_write = _read_nml_int(
        idir / "param.nml", "nhot_write")
    if nhot_write is None:
        print(f"  ERROR {group_id}: could not read "
              f"nhot_write from {idir / 'param.nml'}")
        return False
    ...
    if diag_enabled:
        diag_sbatch = str(
            _render_diag_sbatch(
                cfg, mdir, rdir, config_dir))
        diag_vars_manifest = str(
            rdir / "diag_vars.manifest")
        diag_nvar = max(
            len(cfg.get("diag_run_vars", [])), 1)
    ...
    _render_auto_hotstart(rdir, {
        ...
        "COMBINE_OUTPUT_EXE":     str(
            outdir / combine_output_exe),
        "COMBINE_OUTPUT_NRANKS":  combine_nranks,
        "COMBINE_OUTPUT_SBATCH":  str(
...
def run_setup_run(cfg: dict, config_dir=None):
    ...
    if not failed:
        ...
        print("  Next: enable submit_run and run inside "
              "screen/tmux:")
